...
const TaskCard: React.FC<TaskCardProps> = ({
  ...
  return (
    <>
      <div ref={cardRef} data-row-id={`task-${id}`} onClick={() => setExpanded(!expanded)}
        className={`group relative card-cozy transition-all cursor-pointer ${isDragging ? 'opacity-40' : ''}`}
        ...
        {expanded && (
          <div className="px-3 pb-3 animate-fade-in" onClick={(e) => e.stopPropagation()}>
            <span className={`inline-block text-xs font-bold px-2 py-0.5 rounded-full ${pConfig.color} text-white mb-2`}>{pConfig.label}</span>
            {description && <p className="text-sm text-[var(--text-muted)] mb-2 leading-relaxed">{description}</p>}
            <p className="text-xs text-[var(--text-muted)] italic mb-3">Due {format(taskDate, 'EEEE, MMMM d, yyyy')}</p>
            <div className="flex items-center gap-1 mb-3">
              {(['TO_DO', 'IN_PROGRESS', 'DONE'] as const).map((s) => (
                <button key={s} onClick={() => handleStatusChange(s)} disabled={status === s}
                  className={`text-xs font-bold px-2.5 py-1 rounded-xl transition-colors ${
                    status === s ? 'bg-[var(--primary)] text-white cursor-default' : 'text-[var(--text-muted)] hover:bg-[var(--surface-alt)] hover:text-[var(--text)]'
                  }`}>{statusLabels[s]}</button>
              ))}
            </div>
            <div className="flex items-center gap-2 border-t-2 border-[var(--border)] pt-2">
              <Link href={`/tasks/${id}/edit`} className="flex items-center gap-1 text-xs font-bold text-[var(--primary)] hover:text-[var(--primary-dark)] transition-colors" data-action="edit-task">
                <Pencil size={13} /> Edit
              </Link>
              <button onClick={handleDelete} className="flex items-center gap-1 text-xs font-bold text-[var(--danger)] hover:opacity-80 transition-colors ml-auto" data-action="delete-task">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 animate-fade-in" data-dialog="confirm-delete">
          <div className="card-cozy p-6 max-w-sm w-full mx-4 animate-slide-up">
            ...
            <div className="flex gap-2 justify-end">
              <button onClick={() => setShowConfirm(false)} className="px-4 py-2 text-sm font-bold text-[var(--text-muted)] hover:bg-[var(--surface-alt)] rounded-xl transition-colors">Cancel</button>
              <button onClick={confirmDelete} className="btn-yarn text-sm" style={{ background: 'var(--danger)', borderColor: 'var(--danger)' }}>Delete</button>
            </div>
          </div>
        </div>
      )}
