...
const DashboardPage: React.FC = () => {
  ...
  // TODO: useEffect fetchUserAndTasks
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="flex items-center justify-between gap-3 mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs bg-[var(--surface-alt)] text-[var(--text-muted)] px-2 py-0.5 rounded-full font-bold">{tasks.length}</span>
              </div>
              <div className="flex items-center gap-2">
                <select id="filter" value={filterType} onChange={(e) => setFilterType(e.target.value)} className="px-3 py-1.5 text-sm rounded-xl border-2 border-[var(--border)] bg-[var(--card-bg)] text-[var(--text)] font-bold focus:outline-none focus:border-[var(--primary)]" data-action="filter-tasks">
                  <option value="date">By Date</option>
                  <option value="priority">By Priority</option>
                </select>
                <Link href="/tasks/create" className="btn-yarn inline-flex items-center gap-1.5 text-sm" data-action="create-task">
                  <Plus size={16} /> New
                </Link>
              </div>
            </div>
                ...
                return (
                  <div key={status} className={`flex flex-col rounded-2xl border-2 border-[var(--border)] border-t-4 ${accentMap[status]} bg-[var(--surface)] p-3`}>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-xs bg-[var(--surface-alt)] text-[var(--text-muted)] px-2 py-0.5 rounded-full font-bold">{columnTasks.length}</span>
                    </div>
